...
serve(async (req) => {
  ...
  try {
    console.log("Sending test payload for Vocal Clean Up Rack to GHL...");

    const testPayload = {
      email: "test@bohemyth.com",
      product_title: "Vocal Clean Up Rack",
      amount_paid: 5.00,
      purchase_date: new Date().toISOString(),
      stripe_payment_id: "pi_test_vocalcleanup_" + Date.now(),
