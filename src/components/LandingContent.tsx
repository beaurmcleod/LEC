...
interface LandingContentProps {
  /** Landing-page identifier for click/purchase attribution (e.g. "home", "tiktok", "youtube"). */
  source: string;
}

export const trackLandingClick = async (
  source: string,
  linkTitle: string,
  linkUrl: string,
) => {
  try {
    ...
    if (!user) return; // RLS requires auth
    await supabase.from("link_clicks").insert({
      link_title: linkTitle.substring(0, 200),
      link_url: linkUrl.substring(0, 500),
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="relative z-10 container max-w-2xl mx-auto px-4 py-16">
        ...
        <FeaturedProducts heading="Shop the Essentials" className="mb-12" source={source} />
          ...
          {links.map((link, index) => {
              ...
              <div className="flex items-center gap-4">
                <div className={`w-20 h-20 rounded-lg overflow-hidden shadow-lg group-hover:scale-105 transition-transform duration-300 flex-shrink-0 flex items-center justify-center ${
                  ...
                  {link.icon ? (
                    ...
                    <img src={link.preview} alt={link.title} className="w-full h-full object-cover" />
                  ) : null}
                </div>
            ...
            return link.external ? (
              <a
                ...
                className={className}
                onClick={() => trackLandingClick(source, link.title, link.url)}
              >
                {linkContent}
              </a>
            ) : (
